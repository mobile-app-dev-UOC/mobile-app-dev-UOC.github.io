...
For instance, the code that is executed by a coroutine (we will introduce this concept in [Section 10.1](/content/10/01-coroutines)), or the listener code when attending an event (see [Section 8.1](/content/08/01-listeners)) is usually written as a lambda function. An example of such a listener would be the following:
